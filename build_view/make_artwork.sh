...
OPTS=(--text-paths --segments=real --segments=straight --real-logos --include-display-only --name-format "* %NAME%") # --debug)
...
for K in vfx vfxsd sd1 sd132; do
  mkdir -p "${OUTPUT}/${K}"
  ${PRG} -l ${K} -io 'panel:' -vp 'panel:vfd:' "${OPTS[@]}" "$@" > "${OUTPUT}/${K}/${K}.lay"
